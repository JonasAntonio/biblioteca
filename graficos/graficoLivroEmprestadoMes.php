...
$sql = "
    SELECT 
        (SELECT count(id_emprestimo) WHERE dataEmprestimo LIKE :mesAtual) AS mesAtual, 
        (SELECT count(id_emprestimo) WHERE dataEmprestimo LIKE :mesPassado) AS mesPassado,
        (SELECT count(id_emprestimo) WHERE dataEmprestimo LIKE :mesRetrasado) AS mesRetrasado
    FROM
        tb_emprestimo
";
